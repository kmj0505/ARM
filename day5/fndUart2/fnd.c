...
void display_fnd ( int N )  // Segment 함수 선언
{
  ...
  {
    N=-N;
   arrayNum[0] = 18; //18번째 인덱스 (0~18로 생각해서 0x40이 나옴)
  }
  else
  {
...
{
  GPIO_Write(GPIOC,(GPIO_ReadInputData(GPIOC)| (0x0f00))); //LED all off //0xf0ff ==>  ~(0x0f00) (바꾸려는 값을 1로 준다)
  GPIO_Write(GPIOC,(GPIO_ReadInputData(GPIOC)& ~(0x0100 << position)));
  GPIO_Write(GPIOC,(GPIO_ReadInputData(GPIOC)& 0xff00)|Font[number]); //하위 8비트는 0을 만들어놓고 사용
  Delay(1);
}

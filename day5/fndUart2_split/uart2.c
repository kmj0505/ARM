...
void Serial2_Send(unsigned char t)
{
  ...
  while (USART_GetFlagStatus(USART2, USART_FLAG_TXE) == RESET); //Reset=비어있다
}

void Serial2_Send_String(char* s)
{
  while( *s != '\0') //null 문자가 나올 때까지 한 바이트씩 보낸다
  {
...
void USART2_IRQHandler(void) //데이터가 들어오면 자동으로 인터럽트가 된다
{
...
void print_2d1(double number)	//정수 2자리, 소수 1자리	        /* floating-point number xx.x */
{ 
        unsigned int i, j;

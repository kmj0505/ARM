...
int main()
{
  ...
  //포트A의 0번핀을 외부 인터럽트 0 과 연결한다. - 라인설정
  ...
  NVIC_Init(&NVIC_InitStructure); //init = 설정하는 것
  
  NVIC_InitStructure.NVIC_IRQChannel = EXTI9_5_IRQn; //5번~9번
  NVIC_Init(&NVIC_InitStructure);
  NVIC_InitStructure.NVIC_IRQChannel = EXTI15_10_IRQn; //10번~15번
  NVIC_Init(&NVIC_InitStructure);

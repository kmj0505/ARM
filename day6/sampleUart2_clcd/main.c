...
int main()
{
  ...
  // 마이크로 프로세서 프로그램은 반드시 무한루프를 돌도록 프로그램 해야만  한다.
  while(1);
}
*/
...
void lcd_char(char s)
{
  GPIO_SetBits(GPIOB, TLCD_RS); //RS=0이면 LCD를 설정하는 명령 실행
  GPIO_Write(DATA, s);
  E_Pulse();
}
...
void lcd_disp(char x, char y)
{
  ...
  if(y==0) GPIO_Write(DATA, x+0x80); //x+0x80 7비트가 주소를 더하게끔
...
{
  lcd_disp(x, y);
  while(*str) lcd_char(*str++);  //주소를 움직인다
}
...
int main()
{
  ...
  while(1)
  {
    ...
    for(i = 0; i < 5 ; i++ )
    {
      disp_ON_OFF(OFF, ON, ON);  // 디스플레이 OFF   -> d=0x00, c=0x02, b=0x01 
      Delay(1000);                      // 0000 1011-> 4) 디스플레이 ON/OFF 제어 부분
                                           // 0000 1DCB -> D=0이면 디스플레이 전체 OFF, C=1이면 커서 나타남, B=1이면 커서 깜빡임 
